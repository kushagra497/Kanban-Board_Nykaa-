...
subtaskRouter.patch('/:subtaskId', auth_middleware_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { title, isCompleted } = req.body;
        if (!userId || !subtaskId) {
            return res.status(400).json({ message: 'User ID or Subtask ID not found in request' });
        }
        ...
        res.status(200).json({ message: 'Subtask updated successfully', subtask: subtaskToUpdate });
    }
    catch (error) {
